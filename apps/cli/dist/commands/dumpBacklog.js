...
exports.createDumpBacklogCommand = createDumpBacklogCommand;
const commander_1 = require("commander");
const data_1 = require("@jira-gamification/data");
function createDumpBacklogCommand() {
    const command = new commander_1.Command('dump-backlog')
        .description('Display the current synthetic backlog')
        .option('-f, --format <format>', 'Output format (json, markdown)', 'markdown')
        .action(async (options) => {
        try {
            const { epics, issues } = data_1.BacklogSeeder.seedBacklog();
...
function generateMarkdownBacklog(epics, issues) {
